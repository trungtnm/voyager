@php
$fieldInCamelCase = \Illuminate\Support\Str::camel($row->field);
@endphp
{{-- If this is a relationship and the method does not exist, show a warning message --}}
@if(isset($options->relationship) && !method_exists( $dataType->model_name, $fieldInCamelCase ) )
    <p class="label label-warning"><i class="voyager-warning"></i> {{ __('voyager::form.field_select_dd_relationship', ['method' => $fieldInCamelCase.'()', 'class' => $dataType->model_name]) }}</p>
@endif
@php
...
<select class="form-control select2" name="{{ $row->field }}[]" multiple>
    @if(isset($options->relationship))
        ...
        @if( method_exists( $dataType->model_name, $fieldInCamelCase ) )
            <?php $selected_values = isset($dataTypeContent) ? $dataTypeContent->{$fieldInCamelCase}()->pluck($options->relationship->key)->all() : []; ?>
            <?php
            $relationshipListMethod = $fieldInCamelCase . 'List';
            if (isset($dataTypeContent) && method_exists($dataTypeContent, $relationshipListMethod)) {
                ...
                $relationshipClass = get_class(app($dataType->model_name)->{$fieldInCamelCase}()->getRelated());
